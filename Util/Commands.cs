...
namespace Nocturne
{
    public class Commands
    {
        ...
        public void Log(object obj, Color color)
        {
            ...
            while ( Nocturne.Draw.DefaultFont.MeasureString(str).X > maxWidth )
            {
                int split = -1;
                for ( int i = 0; i < str.Length; ++i )
                {
                    if ( str[i] == ' ' )
                    {
                        if ( Nocturne.Draw.DefaultFont.MeasureString(str.Substring(0, i)).X <= maxWidth)
                        {
                            split = i;
                        }
                        else
        ...
        internal void UpdateOpen()
        {
            ...
            underscoreCounter += Time.DeltaTime;
        ...
        internal void Render()
        {
            ...
            Nocturne.Draw.SpriteBatch.Begin();

            Nocturne.Draw.Rect( 10, screenHeight - 50, screenWidth - 20, 40, Color.Black * OPACITY );
            if ( underscore )
                Nocturne.Draw.SpriteBatch.DrawString( Nocturne.Draw.DefaultFont, ">" + currentText + "_", new Vector2( 20, screenHeight - 42 ), Color.White );
            else
                Nocturne.Draw.SpriteBatch.DrawString( Nocturne.Draw.DefaultFont, ">" + currentText, new Vector2( 20, screenHeight - 42 ), Color.White );

            if ( drawCommands.Count > 0 )
            {
                int height = 10 + ( 30 * drawCommands.Count );
                Nocturne.Draw.Rect( 10, screenHeight - height - 60, screenWidth - 20, height, Color.Black * OPACITY );
                for ( int i = 0; i < drawCommands.Count; ++i )
                {
                    Nocturne.Draw.SpriteBatch.DrawString( Nocturne.Draw.DefaultFont, drawCommands[i].Text, new Vector2( 20, screenHeight - 92 - ( 30 * i ) ), drawCommands[i].Color );
                }
            }

            Nocturne.Draw.SpriteBatch.End();
        }
        ...
        public void ExecuteCommand( string command, string[] args )
        {
            ...
            {
                commands[command].Action.Invoke( args );
            }
            else
            {
        ...
        [Command("debugrender", "Toggles debug rendering ")]
        private static void ToggleDebugRender()
        {
            if ( Engine.Scene == null )
            {
                Engine.Commands.Log("No current scene!", Color.Yellow );
            }
            else
            {
                Engine.Scene.DebugDrawEntities = !Engine.Scene.DebugDrawEntities;
