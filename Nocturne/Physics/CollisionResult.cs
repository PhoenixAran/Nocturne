...
namespace Nocturne
{
    public struct CollisionResult
    {
        ...
        /// <summary>
        /// the translation to apply the first shape and push the shapes a
        /// apart
        /// </summary>
        public Vector2 MinimumTranslationVector;

        /// <summary>
        /// not used for all collisions types! Check the ShapeCollisions class before
        /// relying on this field!
        /// </summary>
        public Vector2 Point;

        /// <summary>
        /// alters the minimumTranslationVector so that it removes the x-component
        /// of the translation if there was no movement in the same direction
        /// </summary>
        ...
        public void RemoveHorizontalTranslation( Vector2 deltaMovement )
        {
            ...
            if ( Math.Sign(Normal.X) != Math.Sign(deltaMovement.X) || ( deltaMovement.X == 0f && Normal.X != 0f ) )
            {
                ...
                // check some edge cases. make sure we don't have normal.x == 1 and a super small y
                // which will result in a huge fix value since we divide by normal
        ...
        {
            return string.Format( "[CollisionResult] Normal: {0}, MinimumTranslationVector: {1}",
                Normal, MinimumTranslationVector );
        }
